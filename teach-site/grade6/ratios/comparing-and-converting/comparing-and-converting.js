/**
 * This program will generate questions that have to do with adding, subtracting, dividing, multiplying and converting
 * units. Program will check students answers and display whether or not they got it correct or not.
 */
...
$(document).ready(function() {
	...
	var generateJSON = function() {
		...
		Question[5].answer = Math.round((Weight1 * 16 + Weight2) * 100) / 100; //16ozs = 1ib ... appleWeight*16 +
	                                                                           // plumWeight
		Question[5].otherAnswer1 = Math.round((Weight1 + Weight2 / 16) * 100) / 100; //1ib = 16ozs ... appleWeight +
	                                                                                 // plumWeight/16
		...
		Question[6].answer = Math.round((Weight1 * 1000 / Weight2) * 100) / 100; //1000g = 1kg ... orangeWeight*1000 /
	                                                                             // grapeWeight
		Question[6].otherAnswer1 = Math.round((Weight1 / (Weight2 / 1000)) * 100) / 100; // 1kg = 1000g ...
	                                                                                     // orangeWeight /
	                                                                                     // (grapeWeight/1000)
		...
		Question[7].answer = Math.round((Weight1 * 453.59237 * Weight2) * 100) / 100; // 453.59237gs = 1ib ...
	                                                                                  // appleWeight*453.59237 *
	                                                                                  // grapeWeight
		Question[7].otherAnswer1 = Math.round((Weight1 * (Weight2 / 453.59237)) * 100) / 100; // 1ib  = 453.59237gs ...
	                                                                                          // appleWeight *
	                                                                                          // (grapeWeight/453.59237);

		Weight1 = Math.round(Math.random() * 2000) / 100;
		Weight2 = Math.round(Math.random() * 100000) / 100;

		Question[8].prompt = Weight1 + "ozs of plums - " + Weight2 + "gs of grapes = ";
		Question[8].answer = Math.round((Weight1 * 28.34952313 - Weight2) * 100) / 100; //28.34952313gs = 1oz ...
	                                                                                    // plumWeight*28.34952313 -
	                                                                                    // grapeWeight
		Question[8].otherAnswer1 = Math.round((Weight1 - Weight2 / 28.34952313) * 100) / 100; //1oz = 28.34952313gs ...
	                                                                                          // plumWeight -
	                                                                                          // grapeWeight/28.34952313
	...
	/**
	 * checks each answer to see if it is correct after some modifications and if it is correct will tell user that
	 * they are correct.
	 */
	var checkAnswer = function() {
		for(var i = 1; i <= 10; i++) {
			...
			/**
			 * the next few steps replace spaces, 'ib', 'oz', 'g', 'kg', 's' with a blank line to eliminate variability
			 * of answers.
			 */
			...
			/**
			 * Real answer is the current question's answer to know what to compare and eliminates repetition and
			 * memory waste
			...
			/**
			 * if checks to see if UserAnswer is equal to RealAnswer. If it is prints out a green block with white
			 * letters that say "You got this right!" else it will print out a blue block with white lettering that
			 * says "Try again, you got it wrong."
			 */
